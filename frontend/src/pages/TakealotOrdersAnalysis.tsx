...
export default function TakealotOrdersAnalysis() {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('/api/v1/takealot/orders/analysis', { params: { days } })
      .then(r => {
        setRows(r.data.data || []);
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(r =>
        ...
        (r.tsin && String(r.tsin).includes(q)),
      );
    }
    return [...data].sort((a, b) => {
      const av = a[sortKey] ?? '';
      const bv = b[sortKey] ?? '';
      if (sortKey === 'sku' || sortKey === 'iwasku' || sortKey === 'product_title' || sortKey === 'last_order_date') {
  ...
  const summaryCards = [
    ...
    { label: 'Total Revenue', value: `R${summary.totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'text-blue-600' },
  ...
  return (
    <div>
      <h1 className="mb-4">Takealot Orders Analysis</h1>
      ...
      <div className="flex gap-3 items-center mb-4">
        <input type="text" placeholder="Search SKU / IWASKU / TSIN / product..."
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b-2 border-slate-200">
                ...
                <th onClick={() => handleSort('tsin')} className="text-left p-2 cursor-pointer select-none whitespace-nowrap text-xs font-semibold">TSIN {sortIcon('tsin')}</th>
                ...
                <th onClick={() => handleSort('avg_unit_price')} className="text-right p-2 cursor-pointer select-none whitespace-nowrap text-xs font-semibold">Avg R {sortIcon('avg_unit_price')}</th>
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.sku ?? `tsin-${r.tsin}`} className="border-b border-slate-100">
                  ...
                  <td className="p-2 text-right text-slate-600">R{Number(r.avg_unit_price).toFixed(2)}</td>
                  <td className={`p-2 text-right font-semibold ${Number(r.total_revenue) > 0 ? 'text-blue-600' : 'text-gray-300'}`}>
                    R{Number(r.total_revenue).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
