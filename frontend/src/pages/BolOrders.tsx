...
const ACCOUNT_LABELS: Record<string, string> = { pera: 'Pera', onebv: 'OneBV' };

export default function BolOrders() {
  const [accounts, setAccounts] = useState<BolAccount[]>([]);
  ...
  useEffect(() => {
    axios.get('/api/v1/bol/settings').then(r => {
      const active = (r.data.accounts || []).filter((a: BolAccount) => a.is_active);
      setAccounts(active);
      if (active.length > 0 && !selectedAccount) setSelectedAccount(active[0].label);
    }).catch(() => {});
  }, []);

  const fetchData = async (page = 1, s = search, account = selectedAccount) => {
    if (!account) return;
    setLoading(true);
    try {
      const res = await axios.get('/api/v1/bol/orders/browse', {
        params: { page, limit: 50, search: s, account },
      });
  ...
  useEffect(() => {
    if (selectedAccount) { setSearch(''); setSearchInput(''); fetchData(1, '', selectedAccount); }
  }, [selectedAccount]);

  const handleSearch = () => {
    setSearch(searchInput);
    fetchData(1, searchInput, selectedAccount);
  };

  return (
    <div>
      <h1 className="mb-4">Bol Orders</h1>
      ...
      <div className="flex gap-3 items-center mb-4">
        <input type="text" placeholder="Search SKU / IWASKU / EAN / order ID / product..."
