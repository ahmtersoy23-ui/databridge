...
async function applyMapping(sku: string, iwasku: string): Promise<void> {
  await pool.query(
    `INSERT INTO bol_sku_mapping (sku, iwasku)
     VALUES ($1, $2)
     ON CONFLICT (sku) DO UPDATE SET iwasku = EXCLUDED.iwasku, updated_at = NOW()`,
    [sku, iwasku],
  );
  await pool.query(
    'UPDATE bol_raw_orders SET iwasku = $1 WHERE sku = $2',
    [iwasku, sku],
  );
}

// GET /api/v1/bol/mappings?filter=all|matched|unmatched&page=1&limit=50&search=
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    let whereClause = 'WHERE bo.sku IS NOT NULL';
    const params: unknown[] = [];
    let idx = 1;

    if (filter === 'matched') whereClause += ' AND bo.iwasku IS NOT NULL';
    else if (filter === 'unmatched') whereClause += ' AND bo.iwasku IS NULL';

    if (search) {
      whereClause += ` AND (bo.sku ILIKE $${idx} OR bo.iwasku ILIKE $${idx} OR bo.product_title ILIKE $${idx})`;
      params.push(`%${search}%`);
      idx++;
    }
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) AS total FROM (
         SELECT bo.sku FROM bol_raw_orders bo
         ${whereClause}
    ...
    const data = await pool.query(
      `SELECT
         ...
         array_agg(DISTINCT c.label ORDER BY c.label) AS accounts,
         m.updated_at AS mapped_at
       FROM bol_raw_orders bo
       JOIN bol_credentials c ON c.id = bo.account_id
       LEFT JOIN bol_sku_mapping m ON m.sku = bo.sku
       ${whereClause}
       GROUP BY bo.sku, bo.iwasku, m.updated_at
...
router.post('/bulk', validateBody(bulkSchema), async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const m of mappings) {
      await applyMapping(m.sku, m.iwasku);
      upserted++;
    }
...
router.get('/export', async (_req: Request, res: Response) => {
  try {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Bol Mappings');
